...
if not ADMIN_API_KEY or ADMIN_API_KEY == "admin-secret-key-change-me":
    ...
    logger.warning(f"ADMIN_API_KEY not configured or insecure default detected. Generated secure random key for this session: {ADMIN_API_KEY}")
...
async def add_alias_group(group: List[str], api_key: str = Depends(verify_admin_key)):
    ...
    for member in full_group:
        engine.aliases[member] = full_group - {member}
